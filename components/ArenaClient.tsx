  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold" style={{ color: '#c8a84b' }}>The Colosseum</h1>
      </div>

      {/* Your settings */}
      <div className="panel mb-6">
        <h2 className="font-bold mb-3" style={{ color: '#c8a84b' }}>Your Pre-Fight Settings</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-bold mb-2" style={{ color: '#e8d5b0' }}>Daring Level</label>
            <select value={daring} onChange={e => setDaring(e.target.value)}
          ...
          <div>
            <label className="block text-sm font-bold mb-2" style={{ color: '#e8d5b0' }}>
      ...
      <div className="flex gap-2 mb-4">
        {(['fight', 'incoming', 'outgoing'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            style={{
              background: tab === t ? '#c8a84b' : '#1a1410',
              color: tab === t ? '#0d0d0d' : '#8a7a5a',
      ...
      {tab === 'fight' && (
        <div className="space-y-3">
          <input
            ...
            className="w-full px-4 py-2 rounded-lg text-sm mb-1"
            style={{ background: '#0d0d0d', border: '1px solid #3d2e1e', color: '#e8d5b0', fontFamily: 'Georgia' }}
          />
            ...
            return (
              <div key={other.id as string} className="panel flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-bold" style={{ color: '#c8a84b' }}>{other.name as string}</p>
      ...
      {tab === 'incoming' && (
        <div className="space-y-3">
          {incomingChallenges.length === 0 && <p className="text-sm" style={{ color: '#5a4a3a' }}>No incoming challenges. Either you're universally respected or universally ignored.</p>}
            ...
            return (
              <div key={c.id as string} className="panel">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="font-bold" style={{ color: '#c8a84b' }}>{challenger.name as string} challenges you!</p>
                    <p className="text-xs" style={{ color: '#5a4a3a' }}>
      ...
      {tab === 'outgoing' && (
        <div className="space-y-3">
          {outgoingChallenges.length === 0 && <p className="text-sm" style={{ color: '#5a4a3a' }}>No pending challenges. Go threaten someone.</p>}
          {outgoingChallenges.map(c => {
            const challenged = c.challenged as Record<string, unknown>
            return (
              <div key={c.id as string} className="panel text-sm" style={{ color: '#8a7a5a' }}>
                Challenge sent to <span style={{ color: '#c8a84b' }}>{challenged.name as string}</span> — waiting for them to accept.
              </div>
            )
          })}
        </div>
      ...
      {challengeTarget && tab === 'fight' && (
        <div className="fixed inset-0 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.8)' }}>
          <div className="panel max-w-sm w-full space-y-4">
            <h3 className="font-bold text-lg" style={{ color: '#c8a84b' }}>
              Challenge {challengeTarget.name as string}?
            </h3>
            <p className="text-sm" style={{ color: '#8a7a5a' }}>
              You'll fight with Daring: <strong>{daring}</strong> and surrender at <strong>{surrenderAt}%</strong> HP.
              {surrenderAt === 0 && <span style={{ color: '#bf4040' }}> This is a death match.</span>}
            </p>
